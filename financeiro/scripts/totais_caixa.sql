...
begin
    ...
    l_query := 'select sum(nvl(' || p_campo_outros || ', 0))'
            || ' from ( select     
                        case when lower(tipo_lancamento_financeiro) in (''dv'', ''cc'', ''dc'', ''dp'', ''ex'', ''sa'', ''tva'') then
                      ...
                      ||  ' from caixas_cupons
                          ...
                          ' and lower(tipo_lancamento_financeiro) in (''pb'', ''eb'', ''pix'') ' ||
                            ')';
    ...
    return l_valor_outros + l_valor_dinheiro;
end;
