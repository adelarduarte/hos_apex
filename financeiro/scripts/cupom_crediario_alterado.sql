create or replace function "CUPOM_CREDIARIO_ALTERADO" (
...
Begin
	...
	-- >> Setar tipo de documento como 'Crediário'
	Select id
	into l_tipo_documento_id
	from tipos_documentos_financeiros
	where descricao = 'Crediário';
	...
	l_valor_parcela 	   := p_valor / p_parcelas;
	l_valor_ultima_parcela := p_valor - (l_valor_parcela * (p_parcelas -1));


	-->> Criar registro contas a receber, retornando o id.
	l_contas_receber_record.data_emissao := sysdate;

	if p_vencimento is not null then
		l_data_vencimento := p_vencimento;
	else
	...
	for i in 1 .. p_parcelas -1 loop
		...
		-->> Criar contas a receber com valor de cada parcela
		...
		l_data_vencimento := l_data_vencimento + 30;

	end loop;

	-->> Criar contas a receber com valor da última parcela, por conta de possíveis
	-- 		diferenças de centavos
	l_contas_receber_record.valor                   := l_valor_ultima_parcela;
	l_contas_receber_record.saldo                   := l_valor_ultima_parcela;
	l_contas_receber_record.cliente_id              := p_cliente_id;
	l_contas_receber_record.empresa_id              := p_empresa_id;
	l_contas_receber_record.cupom_id 				:= p_cupom_id;
	l_contas_receber_record.data_vencimento 		:= l_data_vencimento;
	...
	-->> Somar no saldo do crediário do cliente
