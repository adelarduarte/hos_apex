create or replace procedure "TPL_CALC_GRUPO_CX" (
...
Begin
    ...
    dbms_output.put_line('-->> Calcular Grupo - Caixa');
    l_qtd_rec := 0;
    ...
    if l_qtd_rec > 0 then
        ...
        -->> Criar os registros no relatório, contas recebidas
        for rec in (
            -->> Listar categorias de um grupo, forma de registros
            select sum(valor_entrada) as valor, mes
            from
            (
    ...
    if l_qtd_rec > 0 then
        ...
        for rec in (
            -->> Listar categorias de um grupo, forma de registros
            select sum(valor_saida) as valor, mes from
            (
