create or replace procedure "DEVOLUCAO_A_VISTA" (
...
Begin
	...
	Begin
		...
		and lower(tipo_lancamento_financeiro) = 'vv';
	Exception
	...
	if l_cupom_original_rec.id > 0 then
	   ...
	   l_total_devolvido := l_cupom_devolvido_rec.dinheiro_valor 
		...
		-- Verificando se existe o contas a receber
		...
		if l_contas_receber_id > 0 then
			...
			if l_tipo_devolucao = 'Parcial' then
				...
				set valor_recebido = l_novo_valor_titulo
			...
			l_contas_pagar_record.valor                   := l_total_devolvido;
			...
			l_contas_pagas_record.valor                   := l_total_devolvido;
			l_contas_pagas_record.valor_pago              := l_total_devolvido;
