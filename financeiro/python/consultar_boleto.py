...
def gravar_situacao_boleto(p_titulo_id, p_situacao):
    u = update(contas_receber).where(contas_receber.c.id == p_titulo_id)
    u = u.values(situacao_cobranca=p_situacao)
...
if json_data:
    ...
    print("Consulta de Boletos: Processo finalizado!")
else:
    print("Sem boletos para consultar.")
